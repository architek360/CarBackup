	...
	var _cacheObjects = function() {
        ...
        */
		...
		_countrySelectorAjaxCallHandler = function() {
			...
			$.jStorage.set('countryCodeSelected', $('.js-login-dropdown select option:selected').val());
        ...
        */
		...
		_onPageLoadCountryChecker = function() {
			var urlParam = window.location.search;

			if ($.jStorage.get('countryCodeSelected') !== null) {

				var countrySelected = $.jStorage.get('countryCodeSelected');
				$.jStorage.deleteKey('countryCodeSelected');
				if (urlParam.indexOf('j_username') != -1) {
					$('.js-login-dropdown select').val(countrySelected).trigger('change');
				}
			}

			if (urlParam.indexOf('j_username') != -1) {
				$('#j_password').addClass('error');
				$("#j_username").val(getParameterByName('j_username'));
			}
