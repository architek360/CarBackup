...
IMPORTANT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Added for additional functionality in Jquery ui slider
*/
			...
			_slide: function(event, index, newVal) {
				...
				if (this.options.values && this.options.values.length) {
					...
					if (this.options.values.length === 2 && this.options.range === true) {
						...
						// lower bound max
						if (index === 0 && this.options.lowMax && newVal > this.options.lowMax) {
							newVal = this.options.lowMax;
							$(this.element).trigger('min_range');
						}

						// lower bound min
						if (index === 0 && this.options.lowMin && newVal < this.options.lowMin) {
							newVal = this.options.lowMin;
							$(this.element).trigger('min_range');
							$($('#carat-slider .ui-slider-handle')[0]).find('.js-tool-tip-detail').fadeIn();
							//$($('#carat-slider .ui-slider-handle')[0]).find('.js-tool-tip-detail').fadeOut();

						}

						// upper bound min
						if (index === 1 && this.options.topMin && newVal < this.options.topMin) {
							newVal = this.options.topMin;
							$(this.element).trigger('max_range');
						}

						// upper bound max
						if (index === 1 && this.options.topMax && newVal > this.options.topMax) {
							newVal = this.options.topMax;
							$(this.element).trigger('max_range');
							$($('#carat-slider .ui-slider-handle')[1]).find('.js-tool-tip-detail').fadeIn();
