...
(function(window, $, template, undefined) {
    ...
    */
	...
	template.RetailerTemplateFiller = function(strTemplate, objData) {
		...
		var tempTagArray = [];
		$.each(objData.tags, function(index, val) {
			tempTagArray[index] = CQ.I18n.getMessage('store.refineTags.' + objData.tags[index]);
		});
		var retailDisplayTag = tempTagArray.toString().replace(/,/gi, '<br>');
		/*Internationalize the filter tags display*/
