        ...
        */
		...
		_createCustomeMethods = function() {
			...
			$.validator.addMethod("valueNotEquals", function(value, element, arg) {
				setTimeout(function() {
					if ($(element).attr('name') === 'fn_state') {
						if (arg === value) {
							$('.js-statename .selector').find('span.error').remove();
							$('.errormessage').html(_objPropertiesMsg.mandatory).closest('.error').css('display', 'initial');
						} else {
							$('.errormessage').html("").closest('.error').css('display', 'none');
						}
					}
				}, 100);
				return arg !== value;
			...
			$.validator.addMethod("expiryDate", function(value, elDate) {
				...
				containerDate.find('.js-month, .js-year').addClass('select-wrapper');
		...
		$form.each(function() {
			...
			form.validate({
				...
				errorPlacement: function(error, element) {
					if (element.hasClass('js-ecspayTermCondition')) {
						element.closest("li").find('label').append(error);
						...
						if (element.attr("type") === "radio") {
							...
							if (element.closest('ul').hasClass('cc-name-selector')) {


								element.closest('.cc-name-selector').append(error);
