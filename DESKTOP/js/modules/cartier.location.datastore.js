	...
	cartier.location.datastore = function(options, ns, win, $, undefined) {
		...
		var _xhrData = [],
           ...
           _urlParam = function(name){
			...
			_loadMoreDate = function() {
				var minVal = _xhrData.length;

				for (var k = minVal; k < minVal + 100; k++) {
					if (typeof _xhrDataParent[k] !== "undefined")
						_xhrData.push(_xhrDataParent[k]);
					else
						break;
				}
				...
				ns.publish('dataChanged',{
					data: _xhrData,
					dataSet : _xhrData
				});
				if (_xhrData.length !== _xhrDataParent.length) {
        ...
        */
			_getTagArray = function(store) {
				var _tempString = store.tags.toLowerCase();
				return _tempString.split(' ');
			},
			/*
			...
			_update = function(refresh) {
				...
				if ($('.tab-entrust-boutique').length) {
					...
					var objXHR = cartier.ajaxWrapper.getXhrObj({
						type: 'GET',
						url: baseUrl + _Gurl + '.json?storelocator=' + $('input[name="currentPagePath"]').val(),
						dataType: 'json',
					});
				} else {
					...
					var objXHR = cartier.ajaxWrapper.getXhrObj({
						type: 'GET',
						url: baseUrl + _Gurl + ".json?l="+_urlParam("l"),
						dataType: 'json',
						...
						if (typeof data.success !== 'undefined' && !data.success) {
							...
							if (parsedData !== false) {
								...
								$.each(parsedData, function(i, store) {
									...
									_xhrDataParent.push(parsedData[i]);
								});
