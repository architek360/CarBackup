...
(function(win, $, billing, undefined) {
	...
	_cache = {},
			...
			if (objXHR) {
				objXHR.done(function(data) {
					...
					if (typeof data.success !== "undefined" && !data.success) {
						...
						$('.js-dynamic-delievery').find('.time-options').addClass('display-none');
						$('.date-options').find('select').on('change', _shippingDateChangeHandler);
        ...
        */
		_boutiqueListChangeOnShipping = function() {
			...
			$('.boutique-address').html($(this).find('option:selected').data("baddress"));
			$('.boutique-contact').html($(this).find('option:selected').data("bcontact"));
		...
		_boutiqueListOnLoadOnShipping = function() {
			...
			$('.boutique-address').html($(".js-quantityselect").find('option:first-child').data("baddress"));
			$('.boutique-contact').html($(".js-quantityselect").find('option:first-child').data("bcontact"));
		},
     ...
     */
		...
		_sendPostCodeOKCallBack = function(data) {
			...
			if (data.isSubmitSuccess === false) {
				...
				$('div.js-billing-section-one')
					.prepend('<div class="error-div">' + data.errorMessage + '</div>');
				$('.error-div').ScrollTo({
					duration: 800,
					offsetTop: 140
				});
        ...
        */
		...
		_shippingDateChangeHandler = function() {
			if ($(this).prop('selectedIndex') !== 0) {
				$(".js-delivery-days").find('li.time-options').removeClass('display-none');
			}

			if ($(this).find("option:selected").data('attr') === "default") {
				$(".js-delivery-days").find('li.time-options').addClass('display-none');
			}
		},
        ...
        */
		...
		_japanConfirmBillingAddress = function() {
			...
			$('.js-building input').val(_XSSFilter($('.js-building input').val()));
			...
			$('.js-conf-building').empty().text($('.js-building input').val());
		},
		/*
        ...
        */
		...
		_reviewOrderDataFiller = function() {
			var teaserData = JSON.parse($.jStorage.get('swsecartlc')).teaserArray;

			$('.js-review-product').each(function(index, el) {
				var refid = $(el).data('tref');
				$(teaserData).each(function(i, j) {
					if (j.teaserID === refid) {
						_fillreviewdata($(el), j);
					}
				});
			});
		},

		_fillreviewdata = function(domSection, data) {

			if (data.hasEngraving && typeof data.engravingInformation !== "undefined") {
				if (data.engravingInformation.lineOne.length || data.engravingInformation.lineTwo.length) {
					domSection.find('.js-engraving-info').removeClass('display-none');
					var engravingString = data.engravingInformation.lineOne + " " + data.engravingInformation.lineTwo + "  [";
					if (data.engravingInformation.textStyle) {
						engravingString = engravingString + CQ.I18n.getMessage('engraving.font.cursive') + "]";
					} else {
						engravingString = engravingString + CQ.I18n.getMessage('engraving.font.bold') + "]";
					}
					domSection.find('.js-engraving-info').find('.product-filter__right').text(engravingString);
				}
			}

			if (data.hasBracelet && typeof data.braceletInfo !== "undefined") {
				if (data.braceletInfo.lineOne.length) {
					domSection.find('.js-bracelet-info').removeClass('display-none');
					var braceletString = data.braceletInfo.lineOne + " " + CQ.I18n.getMessage('bracelet.unit.heading');

					if (data.braceletInfo.textStyle) {
						braceletString = braceletString + ', ' + CQ.I18n.getMessage('bracelet.fitType.exact');
					} else {
						braceletString = braceletString + ', ' + CQ.I18n.getMessage('bracelet.fitType.loose');
					}
					domSection.find('.js-bracelet-info').find('.product-filter__right').text(braceletString);
				}
			}

			if (data.hasGiftCard && typeof data.giftCardInfo !== "undefined") {

				if (data.giftCardInfo.cardMessage.length || data.giftCardInfo.receiveBlank) {

					domSection.find('.js-message-info ').removeClass('display-none');
					var messageString = data.giftCardInfo.cardMessage;
					domSection.find('.js-message-info').find('.product-filter__right').text(messageString);
				}
			}
		}
	...
	billing.init = function() {
		...
		if ($('.js-review-product').length)
			_reviewOrderDataFiller();
