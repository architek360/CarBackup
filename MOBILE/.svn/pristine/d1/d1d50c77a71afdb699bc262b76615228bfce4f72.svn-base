...
/*			inputFiledElement.attr('disabled', inputDisabled);
    ...
    */
	var _productFilter = function() {
		...
		if($("select[name='fn_max']").length) // If price filter is there
	...
	productlist.init = function() {
		...
		if (localStorage.getItem('plpviewallselected') !== '' && (!$('.js-search-form').length)) {
			if (localStorage.getItem('plpviewallselected') === 'false') {
				...
				$('.slide_switch').find('input[name="fn_radiocategory"][value=false]').trigger('click');

			} else {
				//$('.slide_switch').find('input[name="fn_radiocategory"][value=true]').trigger('click');
