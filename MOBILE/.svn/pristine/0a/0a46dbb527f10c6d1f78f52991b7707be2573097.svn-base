...
/*!
 * 
 * This file contains a plugin to make a responsive slider. Swipe issue was added manually.
 *
 ...
 * USAGE : 
 *  
 * 1 -ResponsiveSlides.js is a tiny jQuery plugin that creates a responsive slider using elements inside a container.
 * 
 * 2- Intialize ResponsiveSlides with plugname responsiveSlides
 *
 * 3- Options to customise 
 *
...
/*Blank function to be overridden in analytic.js*/
...
(function($, window, i) {
	$.fn.responsiveSlides = function(options) {
		...
		var settings = $.extend({
			...
			"touchThreshold": 150, // Integer: Amount of swipe distance to trigger a slide change, in pixels
			"prevText": "Previous", // String: Text for the "previous" button
			"nextText": "Next", // String: Text for the "next" button
			"maxwidth": "", // Integer: Max-width of the slideshow, in pixels
		...
		return this.each(function() {
			...
			var $this = $(this),
				...
				slideTo = function(idx) {
					settings.before(idx);
					/*function call to register change of slide evemt*/
					slideTicker();
					window.scrollBy(0, -1);
					window.scrollBy(0, 1);
					...
					if (supportsTransitions) {
						$slide
							.removeClass(visibleClass).addClass(invisibleClass)
							.css(hidden)
							.eq(idx)
							.addClass(visibleClass).removeClass(invisibleClass)
							.css(visible);
						index = idx;
						setTimeout(function() {
							settings.after(idx);
			...
			if ($slide.size() > 1) {
				...
				if (settings.nav) {
					var navMarkup =
						"<a href='#' class='" + navClass + " prev'><span>" + settings.prevText + "</span></a>" +
						"<a href='#' class='" + navClass + " next'><span>" + settings.nextText + "</span></a>";
			...
			if (settings.touch) {
				...
				$(".carousel").hammer({
					...
					if (event.gesture.direction === Hammer.DIRECTION_LEFT) {
						...
						/*                 
						 */ //alert('right');
					} else if (event.gesture.direction === Hammer.DIRECTION_RIGHT) {
						slideTo(prevIdx);
						selectTab(prevIdx);
						/*                    */
						//alert('left');
