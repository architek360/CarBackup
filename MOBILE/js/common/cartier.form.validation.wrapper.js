...
(function(window, $, formValidationWrapper, undefined) {
	...
	_objCache,
        ...
        */
		_setDefualtMsg = function() {
			_objMessages = {
				...
				fn_state: {
					required: _objPropertiesMsg.mandatory,
					valueNotEquals: _objPropertiesMsg.mandatory
				},
        ...
        */
		...
		_createCustomeMethods = function() {
			...
			$.validator.addMethod("expiryDate", function(value, elDate) {
				...
				if (month === -1 || year === -1 || year === "default" || month === "default") {
		...
		_removeSpecificRules = function() {
			...
			if (window.countryCode === "jp" || window.countryCode === "JP" || window.countryCode === "Jp" || window.countryCode === "jP") {
				if ($('.billing-form-japan').length && $('input[name="fn_mobileno"]').length) {
					$('input[name="fn_mobileno"]').rules('remove', 'required');
				}
			}
		},
		_addValidationForECS = function(argument) {
			...
			var paymentCvv = $("#payment_card_cvv");
			if (paymentCvv.length) {
				paymentCvv.rules("add", {
					required: true,
					number: true,
					cvvNum: true
				});
			...
			var paymentCardName = $("#payment_card_name");
			if (paymentCardName.length) {
				paymentCardName.rules("add", {
					required: true
				});
			}
		},
        ...
        */
		_customFormBuilderValidation = function() {
			...
			$.validator.addMethod("cardTypeMatch", function(value, element) {
				if ($(".js-ecs-payment-form input[type='radio']:checked").data('cardname') === null) {
					return false;
				}
				if ($(".js-ecs-payment-form input[type='radio']:checked").data('cardname').toLowerCase() === cartier.common.getCardType(value).name) {
                    return true;
				} else {
					return false;
				}
			...
			$.validator.addMethod("cvvNum", function(value, element) {
				...
				if ($(".js-ecs-payment-form input[type='radio']:checked").data('cardname').toLowerCase() === "amex") {
					if (value.length === 4)
						return true;
					else
						return false;
			...
			$.validator.addMethod("mailConfirm", function(value, element) {
				...
				if ($('#piform_emailaddress').length) {
					previousEmail = $('#piform_emailaddress');
				}
				return $(previousEmail).val() === value;
			}, _objPropertiesMsg.sameEmailRequired);
		...
		$form.each(function() {
			...
			form.validate({
				...
				errorPlacement: function(error, element) {
					if (element.attr('id')=='termCondition') {
						element.closest(".group-wrapper").append(error);
    ...
    */
	formValidationWrapper.init = function() {
		...
		if ($('.js-ecs-payment').length)
			_addValidationForECS();
	};
